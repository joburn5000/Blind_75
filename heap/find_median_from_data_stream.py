...
class MedianFinder:
    ...
    def findMedian(self) -> float:
        self.nums.sort()
        if len(self.nums) % 2: # odd
            return self.nums[int(len(self.nums)/2)]
        else:
            return self.nums[int(len(self.nums)/2-1/2)]/2 + self.nums[int(len(self.nums)/2)]/2
...
If the length of the array is odd, we return the center value. If the 
length is even, we return the average between the two middle values.
